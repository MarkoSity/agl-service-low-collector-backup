...
static int memory_read(user_data_t *tmp) /* {{{ */
{
  value_t v[1];
...
int module_register(size_t plugin_index) {
  ...
  if(plugin_register_complex_config("memory", memory_config, plugin_index))
  {
    return -1;
  }

  if(plugin_register_init("memory", memory_init, plugin_index))
  {
    return -1;
  }

  if(plugin_register_read("memory", memory_read, plugin_index))
  {
    return -1;
  }
