...
static void cpu_commit_one(int cpu_num,
                           gauge_t rates[COLLECTD_CPU_STATE_MAX]) {
  gauge_t sum;
  ...
  for (size_t state = 0; state < COLLECTD_CPU_STATE_ACTIVE; state++) {
    ...
    submit_percent(cpu_num, (int) state, percent);
  }
} /* }}} void cpu_commit_one */
...
int cpu_stage(size_t cpu_num, size_t state, derive_t d,
                     cdtime_t now) /* {{{ */
{
  int status;
  cpu_state_t *s;
...
static int cpu_read(user_data_t *tmp) {
  cdtime_t now = cdtime();
