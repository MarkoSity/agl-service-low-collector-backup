...
json_object *write_json(metrics_list_t *metrics_list)
{
  ...
  for(int i = 0 ; i != metrics_list->size ; i++)
  {
    ...
    if(strncmp(metrics_list->metrics[i].plugin_instance, "", strlen(metrics_list->metrics[i].plugin_instance)))
    {
      ...
      {
        /* We store the value we want associated to the type instance */
        json_object_object_add(type_instance, metrics_list->metrics[i].type_instance, json_object_new_double(metrics_list->metrics[i].values->gauge));
        i++;
      }
